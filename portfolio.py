...
def create_movehut_listing(uprn: str, address: str) -> dict:
    try:
        response = requests.post(MOVEHUT_API_URL, json={"uprn": uprn, "address": address}, timeout=10)
...
async def portfolio_summary(q: Optional[str] = Query(None)):
    async with get_conn() as conn:
        ...
        if q:
            ...
            params.append(f"%{q}%")
        sql = f\"\"\"SELECT COUNT(*) AS portfolio_count,
            ...
            FROM er_portfolio p
            JOIN (SELECT uprn, MAX(id) AS max_id FROM er_portfolio GROUP BY uprn) dp ON dp.uprn = p.uprn AND dp.max_id = p.id
            LEFT JOIN vw_mitigation_opportunities o ON o.uprn = p.uprn
            {where_clause}\"\"\"
        row = await conn.fetchrow(sql, *params)
        return {"portfolio_count": int(row["portfolio_count"] or 0), "ends_10": int(row["ends_10"] or 0), "ends_6w": int(row["ends_6w"] or 0), "expired": int(row["expired"] or 0)}
...
async def portfolio_list(q: Optional[str] = Query(None), rows: int = Query(50, ge=10, le=500)):
    async with get_conn() as conn:
        ...
        idx = 1
        if q:
            where_clause = f"WHERE (p.uprn ILIKE ${idx} OR p.address ILIKE ${idx} OR p.council_name ILIKE ${idx} OR p.postcode ILIKE ${idx})"
            params.append(f"%{q}%")
            idx += 1
        params.append(rows)
        sql = f\"\"\"SELECT p.id, p.uprn,
            ...
            CASE COALESCE(v.current_cycle,'ACTIVE') WHEN 'MITIGATION' THEN 'Mitigation' WHEN 'EMPTY' THEN 'Empty' ELSE 'Active' END AS pill_text,
            CASE COALESCE(v.current_cycle,'ACTIVE') WHEN 'MITIGATION' THEN 'pill pill--warning' WHEN 'EMPTY' THEN 'pill pill--danger' ELSE 'pill pill--active' END AS pill_class,
            o.end_date AS opportunity_end_date,
            CASE WHEN COALESCE(v.current_cycle,'ACTIVE') = 'MITIGATION' THEN v.days_remaining
                 WHEN COALESCE(v.current_cycle,'ACTIVE') = 'EMPTY' AND v.cycle_end_date IS NOT NULL THEN EXTRACT(DAY FROM (v.cycle_end_date - CURRENT_DATE))::int
                 WHEN o.end_date IS NOT NULL THEN EXTRACT(DAY FROM (o.end_date - CURRENT_DATE))::int
                 ELSE NULL END AS days_to_end,
            CASE WHEN COALESCE(v.current_cycle,'ACTIVE') = 'MITIGATION' THEN 0
            ...
            CASE WHEN COALESCE(v.current_cycle,'ACTIVE') = 'MITIGATION' THEN 'Mitigation underway'
            ...
            CASE WHEN COALESCE(v.current_cycle,'ACTIVE') = 'MITIGATION' THEN 'na na--window'
            ...
            FROM (SELECT p1.* FROM er_portfolio p1 JOIN (SELECT uprn, MAX(id) AS id FROM er_portfolio GROUP BY uprn) d ON d.uprn = p1.uprn AND d.id = p1.id) p
            LEFT JOIN vw_portfolio_current_cycle v ON v.uarn = p.uarn
            LEFT JOIN (SELECT * FROM (SELECT o2.*, ROW_NUMBER() OVER (PARTITION BY o2.uprn ORDER BY COALESCE(o2.value_score,0) DESC, COALESCE(o2.estimated_saving,0) DESC, o2.end_date DESC NULLS LAST) AS rn FROM vw_mitigation_opportunities o2) r WHERE rn = 1) o ON o.uprn = p.uprn
            {where_clause}
            ORDER BY priority ASC, COALESCE(v.cycle_end_date, o.end_date, '9999-12-31'::date) ASC, COALESCE(o.estimated_saving,0) DESC, p.id DESC
            LIMIT ${idx}\"\"\"
